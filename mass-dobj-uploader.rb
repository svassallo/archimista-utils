...
begin
	...
	if dbname == "" || fond_id == "" || img_dir == "" || user_id == "" || group_id == ""
		...
		Dir.foreach(".") do |item|
			...
			stmcount = db.prepare "SELECT count(*) as count FROM units WHERE root_fond_id = :fond_id AND file_number = :item"
			count = stmcount.execute fond_id, item
		...
		#stmcount.close if stmcount
		#stm.close if stm
		#stmins.close if stmins
		#db.close if db
